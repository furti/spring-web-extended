...
public class CdnExpressionHandler extends UrlGeneratingExpressionHandler
{
	...
	public CdnExpressionHandler(ApplicationConfiguration config,
			CdnConfig cdnConfig)
	{
		super();
	...
	public String process(String value)
	{
		...
		Assert.notNull(entries, "Stack " + value
				+ " not found in CdnConfiguration");
	...
	{
		String link = config.isOptimizeResources()
				? entry.getMinifiedLocation()
				: entry.getLocation();

		int index = link.lastIndexOf(".");

		Assert.isTrue(index > 0, "Value for CDN must contain an ending ["
				+ link + "]");
		...
		{
			throw new IllegalArgumentException("Ending ." + ending
					+ " not known");
		}
	}
