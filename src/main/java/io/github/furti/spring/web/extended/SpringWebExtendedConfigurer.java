...
public interface SpringWebExtendedConfigurer {
    ...
    /**
     * You can add or remove scanners to be used for scanning static folders here. A
     * scanner has a prefix that allows it
     * to be referenced in resource locations. e.g. to scan a folder on the
     * classpath you can use a resource string like
    ...
    /**
     * Allows you to configure some well knonwn mime types in you application. If a
     * mime type is not found in this list,
     * the servlet context will be asked for a mime type. So you can use this map to
     * override standard mime types or add
     * mime types that the servlet context does not know.
     * <p>
     * Key is the file ending with a leading dot. Value is the mime type for this
     * file ending.
     * </p>
     *
    ...
    /**
     * Allm mime types in this list will have caching headers added to the response
    ...
    /**
     * Configures content escape handlers that are used by the template engine to
     * escape expressions for the given mime
    ...
    /**
     * Configures resource types used by the {@link StaticFolderCache} to handle
     * resources.
     *
    ...
    /**
     * Add some locale sources to use for the application. The locale sources are
     * asked in the order they where
     * registered. The first locale that is returned will be used. To use the locale
     * set by the locale source you can
     * simply inject the {@link LocaleContext} bean into your spring beans. If no
     * localesource is registered the request
     * locale will be used.
     *
    ...
    /**
     * Define the mime types, that are subject to compression. Responses matching
     * one of the mime type in this list will
     * be compressed using a algorithm the client requested when applicable.
     * Clearing the list measn no compression will
     * be performed regardless if the client supports it or not.
     *
     * @param compressableMimeTypes list of mime types that are subject to
     *                              compression
     */
    ...
    /**
     * Let the application override the caching sematics for each available mime
     * type. Per default all cachable
     * mimetypes will get the following Cache-Control header applied: "public,
     * max-age=86400, must-revalidate" This
     * means they will be cached for 24 hours and then the browser sends a
     * validation request.
     *
